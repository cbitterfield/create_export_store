...
''' % (program_shortdesc, str(__date__))
    ...
    try:
        ...
        logger.info('Connecting to schema {0} and loading data to table {1}'.format(schema,output))
        try:
            cnx = sqlite3.connect(schema)
        ...
        for child in root:
            ...
            variables = '?,'* len(col_array)
            
            query = "insert into " + output + "("  + columns + ")" + "VALUES" + "(" + variables[:-1] + ")"
            logger.info(query)
            logger.info(query)
            cursor = cnx.cursor()
